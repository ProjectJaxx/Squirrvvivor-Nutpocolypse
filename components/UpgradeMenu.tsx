...
export const UpgradeMenu: React.FC<UpgradeMenuProps> = ({ upgrades, onSelect }) => {
  return (
    <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm z-50 p-4">
      <div className="bg-gray-800 border-4 border-amber-600 rounded-lg p-4 md:p-8 max-w-4xl w-full shadow-2xl transform scale-100 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {upgrades.map((upgrade, idx) => (
            <button
              ...
              className="group relative bg-gray-700 hover:bg-gray-600 border-2 border-gray-600 hover:border-amber-400 rounded-xl p-4 md:p-6 flex flex-row md:flex-col items-center text-left md:text-center transition-all duration-200 hover:-translate-y-1"
            >
              ...
              <div className="flex flex-col flex-1">
                  ...
                  <p className="text-xs md:text-sm text-gray-300 mb-2">{upgrade.description}</p>
