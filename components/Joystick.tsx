  ...
  return (
    ...
    >
      {active && (
        ...
        >
          <div className="w-12 h-12 bg-orange-500/80 rounded-full shadow-lg" style={{ transform: `translate(${position.x}px, ${position.y}px)` }} />
        </div>
      )}
      {!active && (
        <div className="absolute bottom-4 w-full text-center text-white/20 text-sm font-bold pointer-events-none animate-pulse">TOUCH ANYWHERE TO MOVE</div>
      )}
    </div>
  );
};
