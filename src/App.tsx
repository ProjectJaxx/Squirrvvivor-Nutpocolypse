...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    char.startingCompanions = [...(char.startingCompanions || [])];
    char.startingWeapons = [...(char.startingWeapons || [])];
    ...
    BASE_UPGRADES_LIST.forEach(def => {
        ...
        if (level > 0) {
            ...
            } else if (def.statKey === 'unlock_companion' && def.companionType) {
                // Add companion if not already present
                if (!char.startingCompanions!.includes(def.companionType)) {
                    char.startingCompanions!.push(def.companionType);
                }
            } else if (def.statKey === 'unlock_weapon' && def.weaponConfig) {
                // Add weapon if not already present
                const exists = char.startingWeapons!.some(w => w.type === def.weaponConfig!.type);
                if (!exists) {
                    char.startingWeapons!.push({ ...def.weaponConfig });
                }
            }
        }
    });
  ...
  const startGame = () => {
    ...
    setHudStats({ score: 0, kills: 0, nuts: 0, time: 0, wave: 1, player: undefined, boss: null, extractionTimer: 0 });
  };
            ...
            <div className="absolute inset-0">
                ...
                {appState === 'GAME' && hudStats.player && (
                    <GameHUD 
                        ...
                        onPause={togglePause}
                        boss={hudStats.boss}
                        extractionTimer={hudStats.extractionTimer}
                    />
                )}
